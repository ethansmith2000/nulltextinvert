...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, prompts, num_steps: int, tokenizer,
                 ...
                 local_blend: Optional[LocalBlend], device=None, dtype=None):
...
def get_equalizer(text: str, tokenizer, word_select: Union[int, Tuple[int, ...]], values: Union[List[float],
                                                                                     Tuple[float, ...]], device=None, dtype=None):
    if type(word_select) is int or type(word_select) is str:
        word_select = (word_select,)
    equalizer = torch.ones(1, 77).to(device).to(dtype)
